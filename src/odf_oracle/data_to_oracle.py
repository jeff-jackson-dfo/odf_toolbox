def data_to_oracle(odfobj, user, pwd, hoststr, infile):
    """
    -----------------------------------------------------------------------
    DATA_TO_ORACLE: Load the Cruise_Header and Event_Header
    information from the input ODF structure into Oracle.
    ...
    @summary: Load the Cruise_Header and Event_Header information from the
               input ODF structure into Oracle.

    Usage:  data_to_oracle(odfobj, user, password, hoststr, infile)

    Input:
       odfobj: An ODF structure array.
    ...
    Example:  data_to_oracle(ODF, 'jj', '1234', 'host:21/dbname', 'CTD_98023_33_1_DN.ODF')
    ...
    Updates:
      ...
      Jeff Jackson (21-OCT-2014)
      - Finished getting the function working.

      Jeff Jackson (23-MAR-2016)
      - Fixed an error. The code incorrectly handled missing SYTM channels by not outputting any data at all.

      Jeff Jackson (05-APR-2016)
      - Fixed an error with the code that removed parameters that only contained null values.

      Jeff Jackson (16-JUN-2016)
      - Fixed an error with the code that resulted when the parameter code was only four characters long and did not
      contain the sensor number associated with it; e.g. 'TEMP_01'

      Jeff Jackson (21-JAN-2017)
      - Added code to load the quality flag associated with a data value otherwise a quality flag of 0 is added.

      Jeff Jackson (28-NOV-2017)
      - Added the input argument "infile" which is the ODF file currently being loaded.
    ...
    from remove_parameter import remove_parameter
    ...
    param_headers = odfobj.get('PARAMETER_HEADER')

    # Retrieve the data from the input ODF structure.
    # data1 = odfobj.get('DATA')
